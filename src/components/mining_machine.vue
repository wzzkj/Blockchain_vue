<template>
  <div>
    ...
    >
      ...
      <el-table-column
        v-for="column in tableColumns.filter((col) => col.prop !== 'isBuy')"
        :key="column.prop"
        :prop="column.prop"
        :label="column.label"
        ...
        min-width="120"
      />
      <el-table-column label="是否开启" prop="isBuy" width="120" align="center">
        <template #default="{ row }">
          <span :style="{ color: row.isBuy ? 'green' : 'red' }">
            {{ row.isBuy ? "开启" : "关闭" }}
          </span>
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <template v-for="field in formFields" :key="field.prop">
          ...
          <!-- 特殊处理：周期时长 (标签动态变化) -->
          <el-form-item
            v-else-if="field.prop === 'cycleDuration'"
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        cycleDuration: [
          { required: true, message: "周期时长不能为空", trigger: "blur" },
        ],
        yieldRate: [
  ...
  computed: {
    formFields() {
      return this.tableColumns.filter(
        (col) => !EXCLUDED_FORM_FIELDS.includes(col.prop)
      );
    },
  },
  methods: {
    onCycleTypeChange(newType) {
      this.cycleDurationLabel =
        newType === "DAY" ? "周期时长(天)" : "周期时长(小时)";
    },

    cycleTypeFormatter(row, column, cellValue) {
      const option = this.cycleTypeOptions.find(
    ...
    async handleSwitchChange(row, val) {
      try {
        const payload = { ...row, isBuy: val };
        const result = await updateMiningMachine(payload);
        if (result && (result.code === 200 || result.success)) {
          ElMessage.success(`已${val ? "开启" : "关闭"}矿机 "${row.name}"`);
        } else {
